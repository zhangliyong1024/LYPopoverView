...
- (void)showToPoint:(CGPoint)toPoint
{
    ...
    if (currentH > maxHeight) { // 如果弹窗高度大于最大高度的话则限制弹窗高度等于最大高度并允许tableView滑动.
        ...
        if (!_isUpward) { // 箭头指向下则移动到最后一行
            [_tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:self.actions.count - 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:NO];
        }
    }
...
- (NSArray<PopoverAction *> *)actions {
    if (self.dataSource) {
        NSInteger count = [self.dataSource numberOfRowsInMenu:self];
        NSMutableArray *arr = [NSMutableArray new];
        for (int i = 0; i < count; i++) {
            PopoverAction *action = [self.dataSource actionForRow:i];
...
- (void)showToView:(UIView *)pointView {
    [self showToView:pointView withActions:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSInteger count;
    if (self.dataSource) {
        action = [self.dataSource actionForRow:indexPath.row];
        count = [self.dataSource numberOfRowsInMenu:self];
    }
    else {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    [UIView animateWithDuration:0.25f
                     ...
                     completion:^(BOOL finished) {
                         if (self.delegate) {
                             [self.delegate popover:self didSelectRow:indexPath.row];
                         }
                         else {
